...
// تنفيذ العمليات: تعديل الدور، حذف المستخدم، إضافة مستخدم جديد، تعديل بيانات المستخدم
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_POST['delete_user'])) {
        $stmt = $pdo->prepare("DELETE FROM users_table WHERE id_user = ?");
        $stmt->execute([$_POST['user_id']]);
    }

    if (isset($_POST['add_user'])) {
        $stmt = $pdo->prepare("INSERT INTO users_table (username, email, password, role) VALUES (?, ?, ?, 'user')");
        $stmt->execute([$_POST['new_username'], $_POST['new_email'], password_hash($_POST['new_password'], PASSWORD_DEFAULT)]);
    }
...
$stmt = $pdo->query("SELECT id_user, username, email, role FROM users_table");
...
<nav class="navbar navbar-expand-lg navbar-light bg-light" style="background: linear-gradient(to left ,#547792,#ffffff);display: flex;position:fixed;width: 100%;top: 0px ; z-index: 99;">
    ...
    <h2>إدارة المستخدمين - مسؤول النظام</h2>
    ...
    <table class="admin_table">
        <tr>
            ...
            <th class="admin_table_th">الترتيب</th>
        ...
        <tr >
            <td class="admin_table_th"><?= htmlspecialchars($user['role']) ?></td>
            <td class="admin_table_th"><?= htmlspecialchars($user['email']) ?></td>
            <td class="admin_table_th"><?= htmlspecialchars($user['username']) ?></td>
            <td class="admin_table_th"><?= htmlspecialchars($user['id_user']) ?></td>
            <td class="admin_table_th">
                ...
                <form method="POST">
                    ...
                    <button type="submit" name="update_role">حدد الدور</button>
                </form>
            </td>
            <td class="admin_table_th">
                <!-- زر حذف الحساب -->
